...
def test_request_matched_endpoint():
    with TestMock() as mock_test:
        try:
            with NetworkBlocker():
                urllib.request.urlopen('http://127.0.0.1/test/abc/', timeout=0).read()
            fail('Request should raise HTTPError')
        except urllib.error.HTTPError as e:
            assert e.code == 418
            assert e.read() == b'{"id": "abc"}'
        mock_test.test.request_mock.assert_called_once()
        assert mock_test.test.request_mock.call_args[0][0] == {'test_id': 'abc'}
...
def test_request_override_response():
    with TestMock() as mock_test:
        ...
        with NetworkBlocker():
            response = urllib.request.urlopen('http://127.0.0.1/test/abc/', timeout=0)
...
def test_request_override_removed_nested_mock():
    with TestMock() as mock_test1:
        ...
        assert mock_test1.test.request_mock.call_args[0][0] == {'test_id': 'abc'}
